...
class TestChecker:
    ...
    def check(soldir, asttree, num):
        ...
        if type(asttree.decl[0]) is VarDecl:
            var_decl = [x for x in asttree.decl if type(x) is VarDecl]
            result = checker.typeCheck(var_decl[0].varType, var_decl[1].varType)
            dest.write(str(result))
            dest.close()
        else:
            lhs_func = FunctionType([x.varType for x in asttree.decl[0].param],asttree.decl[0].returnType)
            rhs_func = FunctionType([x.varType for x in asttree.decl[1].param],asttree.decl[1].returnType)
            result = checker.typeCheck(lhs_func, rhs_func)
            dest.write(str(result))
            dest.close()
        # try:
        #     res = checker.check()
