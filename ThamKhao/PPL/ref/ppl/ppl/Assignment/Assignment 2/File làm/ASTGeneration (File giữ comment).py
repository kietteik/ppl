...
class ASTGeneration(MCVisitor):
    ...
    def visitProgram(self,ctx:MCParser.ProgramContext):
        ...
        # return Program(reduce(lambda x, y: x + self.visit(y), 
        #                 ctx.declaration()[1:], 
    ...
    def visitVar(self,ctx:MCParser.VarContext): 
        #return Id(ctx.ID()).getText() if ctx.getChildCount() == 1 else ArrayCell(self.visit(ctx.ID()).getText(),self.visit(ctx.INTLIT()).getText())
    ...
    def visitParaList(self,ctx:MCParser.ParaListContext):
        ...
        #return reduce(lambda x, y: x + self.visit(y), ctx.paradecls()[1:], self.visit(ctx.paradecls()[0]))
    ...
    # def visitBlockStmt(self,ctx:MCParser.BlockStmtContext):
        ...
        #     lst.append(self.visit(ctx.getChild(i+1)))   
        # return Block(lst)
    ...
    # def visitExp7(self, ctx:MCParser.Exp3Context):
    #     if ctx.getChildCount() == 1:
