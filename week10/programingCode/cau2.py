...
class StaticCheck(Visitor):
    ...
    def visitBlock(self,ctx:Block,o): 
        oBlock = {}
        env = [x.accept(self, oBlock) for x in ctx.decl]
        oBlock2 = oBlock.copy()
        oBlock2.update(o)
        oBlock2.update(oBlock)
        env2 = [x.accept(self, oBlock2) for x in ctx.stmts]
        for k,v in oBlock2.items():
            if k in oBlock and k in o:
                continue
            o[k]=v
